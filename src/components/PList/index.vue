      ...
      <template #content>
        <view v-if="hasData" class="card-container">
          ...
          >
            ...
            <text v-if="item.$columnStatus" :class="['card__status', item.$columnStatusType]">
              {{ item.$columnStatus }}
            </text>
            <view class="card__main">
              <view class="card__content">
                <view class="card__content-left">
                  <van-image
                    v-if="item.$columnImgSrc"
                    v-bind="config.imgProps"
                    :style="config.imgStyle"
                    :src="item.$columnImgSrc"
                  />
                </view>
                <view class="card__content-right">
...
watch(
  ...
  () => {
    ...
    const { titleProp, personProp, timeProp, statusProp, statusTypeProp, imgProp } =
      mergeProps.value
    deepClone(props.data).forEach(o => {
      o.$columnID = guid()
      o.$columnTitle = getValueByKey(titleProp, o)
      o.$columnPerson = getValueByKey(personProp, o)
      o.$columnTime = getValueByKey(timeProp, o)
      o.$columnImgSrc = getValueByKey(imgProp, o)
...
function handleListClick(e: IListData) {
  emits('click', e)
}
